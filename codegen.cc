...
{
    sif->exp_->accept(this);//Beginning of if test
    code.add(I_JR_IF_FALSE);//Jump past the body
    code.add(0);
    int patchloc = code.pos()-1;
...
void CodeGen::visitSIfElse(SIfElse *sif)
{
    ...
    sif->stm_1->accept(this);//Body 1
    code.add(I_JR);
    code.add(0);
    int endifloc = code.pos() - 1;
    code.at(elseloc) = code.pos() - (elseloc -1);

    sif->stm_2->accept(this);//Body 2
    code.at(endifloc) = code.pos() - (endifloc -1);
}

//aded code for for-scoped for loop
void CodeGen::visitSFor2(SFor2 *sfor2)
{
    int loopLocation=code.pos();
    sfor2->exp_1->accept(this);
    code.add(I_JR_IF_FALSE);
    code.add(0);
    int pathLocation=code.pos() - 1;
    sfor2->stm_->accept(this);
    sfor2->exp_2->accept(this);
    code.add(I_JR);
    code.add(loopLocation-(code.pos()-1));
    code.at(pathLocation)=code.pos()-(pathLocation-1);
}
//added code for repeat-until
void CodeGen::visitSRepeat(SRepeat *srepeat)
{
    int looploc = code.pos(); // Beginning of test
    srepeat->exp_->accept(this);
    code.add(I_JR_IF_TRUE);  // Jump past the body.
    code.add(0);
    int patchloc = code.pos() - 1;
...
void CodeGen::visitGlobal(Global *global)
{
    ...
    //insert Ident (it is currid) into the symbol table
    Ident global_name = currid;
    
    if (symbols.exists(global_name))
	throw Redeclared(global_name);

    symbols.insert(Symbol(global_name,currtype,3+symbols.numvars()));
}
